...
public class SanPhamDAO extends AbstractAccessData<SanPham> implements ISimpleAccess<SanPham, Integer> {
    ...
    public boolean insert(SanPham sanPham) {
        return executeUpdate(
                "INSERT INTO SanPham(TENSP, MOTA, SOLUONG, HINHANH, GIATIEN, MALOAI, MANCC, IS_DELETED) VALUES(?,?,?,?,?,?,?,?)",
                sanPham.getTenSP(), sanPham.getMoTa(), sanPham.getSoLuong(), sanPham.getHinhAnh(), sanPham.getGiaTien(),
                sanPham.getMaLoai(), sanPham.getMaNCC(), sanPham.isDeleted());
    }
    ...
    public boolean update(Integer maSanPham, SanPham sanPham) {
        return executeUpdate(
                "UPDATE SanPham SET TENSP = ?, MOTA = ?, SOLUONG = ?, HINHANH = ?, GIATIEN = ?, MALOAI = ?, MANCC = ?, IS_DELETED = ? WHERE MASP = ?",
                sanPham.getTenSP(), sanPham.getMoTa(), sanPham.getSoLuong(), sanPham.getHinhAnh(), sanPham.getGiaTien(),
                sanPham.getMaLoai(), sanPham.getMaNCC(), sanPham.isDeleted(), maSanPham);
    }
    ...
    public List<SanPham> selectTenHayMoTa(String findKey) {
        ...
        return executeQueryList("SELECT * FROM SanPham WHERE (MOTA LIKE ? OR TENSP LIKE ?) AND IS_DELETED = 0",
                searchStr, searchStr);
    }
    ...
    public List<SanPham> selectByNhaCungCap(String maNCC) {
        ...
        try {
            ...
            return executeQueryList(
                    "SELECT SANPHAM.* FROM SANPHAM, NHACUNGCAP WHERE SANPHAM.MANCC = NHACUNGCAP.MANCC AND SANPHAM.IS_DELETED=0 AND NHACUNGCAP.TENNCC LIKE ?",
                    searchStr); //SANPHAM.* có nghĩa là: "Tôi chỉ muốn lấy tất cả các cột thuộc về bảng SANPHAM thôi, còn cột của bảng NHACUNGCAP thì đừng lấy
        }
        // Neu chuyen qua dc
    ...
    public List<SanPham> selectByMoney(String minMoney, String maxMoney) {
        return executeQueryList("SELECT * FROM SANPHAM WHERE (GIATIEN >= ? AND GIATIEN <= ?) AND IS_DELETED=0",
                minMoney, maxMoney);
    }
